...
/*   Updated: 2024/04/07 18:57:30 by aarponen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_outfile(t_cmd *cmds)
{
	int	write_fd;

	if (cmds->redir->append == 1)
		write_fd = open(cmds->redir->out, O_WRONLY | O_APPEND | O_CREAT, 0664);
	else
		write_fd = open(cmds->redir->out, O_WRONLY | O_TRUNC | O_CREAT, 0664);
	if (write_fd == -1)
